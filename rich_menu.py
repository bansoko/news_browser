...
class RichMenuManager:
    ...
    def upload_image(self, rich_menu_id, image_path):
        ...
        return requests.post(
            url,
            headers=dict(self.headers, **{"content-type": "image/png"}),
            data=image_file,
            verify=self.verify).json()

    def download_image(self, rich_menu_id, filename=None):
        url = "https://api.line.me/v2/bot/richmenu/%s/content" % rich_menu_id
    ...
    def apply(self, user_id, richmenu_id):
        url = "https://api.line.me/v2/bot/user/%s/richmenu/%s" % (user_id,
                                                                  richmenu_id)
        return requests.post(
            url, headers=self.headers, verify=self.verify).json()
